...
class ChaitanyaAppointmentService(models.Model):
    ...
    def _odoo_appointment_type_values(self):
        ...
        if "schedule_based_on" in AppointmentType._fields:
            values["schedule_based_on"] = "resources"  # changed from "users""
        ...
        if "resource_ids" in AppointmentType._fields:
            resources = self.provider_ids.mapped("resource_id").filtered(lambda r: r.id)
